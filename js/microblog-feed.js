/* Micro.blog feed | robertbirming.com — qt-mb- port */
(async () => {
  ...
  async function fetchReplies(permalink, el) {
    try {
      const res = await fetch(
        ...
        + encodeURIComponent(permalink) + '&format=jsonfeed'
      );
      if (!res.ok) throw new Error();
      const data = await res.json();
      const count = data.items?.length || 0;
      const page = data.home_page_url || permalink;
      el.innerHTML = '<a href="' + page + '" rel="noopener">'
    ...
    items
      ...
      .forEach(item => {
        ...
        const html = item.content_html
          || '<p>' + String(item.content_text)
              .replace(/&/g, '&amp;').replace(/</g, '&lt;') + '</p>';
        const permalink = item.url || '#';
        const date = new Date(item.date_published || Date.now())
        ...
        card.innerHTML =
          ...
          '<div class="qt-mb-feed-meta">' +
            ...
            '<span class="qt-mb-feed-replies">'
              + '<a href="' + permalink + '" rel="noopener">Reply</a></span>' +
          '</div>';
